...
export class Player {
  constructor(name = '小冒险家') {
    ...
    // Per-unit battle progress — survives page refresh / exit
    // { [unitId]: { defeatedWordIds: string[] } }
    this.unitBattleProgress = {};
  ...
  // ========== Unit Battle Progress ==========

  /**
   * Record a word defeated in the current unit battle run.
   * This allows the player to exit mid-battle and resume later.
   */
  recordDefeatedWord(unitId, wordId) {
    if (!this.unitBattleProgress[unitId]) {
      this.unitBattleProgress[unitId] = { defeatedWordIds: [] };
    }
    const progress = this.unitBattleProgress[unitId];
    if (!progress.defeatedWordIds.includes(wordId)) {
      progress.defeatedWordIds.push(wordId);
    }
  }

  /**
   * Clear battle progress for a unit (called when boss is defeated).
   */
  clearUnitProgress(unitId) {
    delete this.unitBattleProgress[unitId];
  }

  /**
   * Get the number of words defeated so far in this unit's current run.
   */
  getUnitDefeatedCount(unitId) {
    const progress = this.unitBattleProgress[unitId];
    return progress ? progress.defeatedWordIds.length : 0;
  }
  ...
  deserialize(data) {
    ...
    // Battle progress (new field — tolerate missing in old saves)
    this.unitBattleProgress = (data.unitBattleProgress && typeof data.unitBattleProgress === 'object')
      ? JSON.parse(JSON.stringify(data.unitBattleProgress))
      : {};
